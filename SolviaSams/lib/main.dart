...
void main() async {
  ...
  // === ĐOẠN CODE GỌI SERVER PYTHON CHẠY NGẦM ===
  try {
    // Nó sẽ tìm thư mục sams_server nằm cạnh file exe của Flutter để chạy
    await Process.start('sams_server\\sams_server.exe', []);
  } catch (e) {
    debugPrint("Không thể khởi động Máy chủ AI: $e");
  }
  // ==============================================

  runApp(const SolviaSAMSApp()); // Hoặc tên Class gốc của bạn
}
                      ...
                      try {
                        ...
                        if (response.statusCode == 200) {
                          ...
                          if (data['status'] == 'success') {
                            ...
                            // ==========================================
                            // ĐÃ FIX: LƯU ROLE VÀO GLOBALS ĐỂ PHÂN LUỒNG
                            // ==========================================
                            globals.currentUserRole = selectedRole; // Tạm lấy từ Option người dùng chọn (Thành viên)
                            if (data['data']['role'] != null) {
                              globals.currentUserRole = data['data']['role']; // Ưu tiên quyền chính xác từ Server trả về
                            }
