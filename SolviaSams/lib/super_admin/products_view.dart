                        ...
                        children: [
                          ...
                          SizedBox(
                              ...
                              child: ElevatedButton.icon(
                                  onPressed: () async {
                                    // Chốt chặn an toàn: Bắt người dùng phải có Dự án trước khi mua gói
                                    if (globals.currentProjectId == 0) {
                                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Vui lòng vào 'Trang chủ' và chọn một dự án trước khi đăng ký gói!"), backgroundColor: Colors.orange));
                                      return;
                                    }
                                    ...
                                    try {
                                      // Đóng gói thông tin hóa đơn
                                      Map<String, dynamic> payload = {
                                        "package_name": tier.name,
                                        "server_specs": tier.serverSpecs,
                                        "storage_capacity": tier.storage,
                                        "max_students": tier.maxStudents,
                                        "max_cameras": tier.maxCameras,
                                        "included_cameras": tier.includedCameras,
                                        "extra_cameras": extraCams,
                                        "monthly_fee": package.monthlyFee,
                                        "total_hardware_price": totalHardwarePrice,
                                        "upfront_payment": upfrontPayment,
                                        "remaining_payment": remainingPayment,
                                        "payment_method": paymentMethod
                                      };

                                      // Bắn lên Server Python
                                      var response = await http.post(
                                          Uri.parse('http://127.0.0.1:8000/api/projects/${globals.currentProjectId}/subscribe'),
                                      ...
                                      if (response.statusCode == 200) {
                                        ...
                                        if (data['status'] == 'success') {
                                          if (context.mounted) Navigator.pop(context); // Đóng luôn bảng Hóa đơn
                                  ...
                                  icon: Icon(Icons.check_circle_rounded, color: Colors.white, size: 18 * theme.fontScale),
                                  label: Text("XÁC NHẬN ĐĂNG KÝ", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13 * theme.fontScale)),
                                  style: ElevatedButton.styleFrom(backgroundColor: Colors.green, padding: const EdgeInsets.symmetric(vertical: 20), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)))
                              )
                          )
                        ],
                      ),
