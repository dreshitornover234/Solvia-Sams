...
class _SettingsViewState extends State<SettingsView> {
  ...
  Future<void> _changePassword() async {
    ...
    try {
      ...
      var response = await http.put(
          Uri.parse('http://127.0.0.1:8000/api/users/$_currentUserId/password'), // Đổi thành 10.0.2.2 nếu dùng máy ảo
  ...
  Future<void> _saveSettings(AppTheme theme) async {
    ...
    try {
      ...
      var response = await http.put(
          Uri.parse('http://127.0.0.1:8000/api/users/$_currentUserId/settings'),
          headers: {"Content-Type": "application/json"},
          body: jsonEncode(payload)
      );
