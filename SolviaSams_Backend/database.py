...
class Project(Base):
    ...
    project_code = Column(String(50), unique=True, index=True, default=lambda: f"SAMS-{uuid.uuid4().hex[:6].upper()}")
    qr_code_data = Column(Text)

    # 2. Thông tin cơ bản
    project_name = Column(String(255), nullable=False)
    project_type = Column(String(50), default="Trường học")
    ...
    attendance_mode = Column(String(100))
    global_rule = Column(String(100))
    bell_schedule = Column(JSON, nullable=True)
    # =========================================================
    # 4. BỔ SUNG MỚI: THÔNG TIN GÓI DỊCH VỤ & THANH TOÁN
    # =========================================================
    package_name = Column(String(150), nullable=True)  # Tên gói (VD: SAMS Cloud Enterprise)
    server_specs = Column(String(150), nullable=True)  # Cấu hình máy chủ
    storage_capacity = Column(String(50), nullable=True)  # Dung lượng lưu trữ

    # Giới hạn của gói
    max_students = Column(Integer, default=0)
    max_cameras = Column(Integer, default=0)
    included_cameras = Column(Integer, default=0)
    extra_cameras = Column(Integer, default=0)

    # Chi tiết Hóa đơn thanh toán
    monthly_fee = Column(Integer, default=0)  # Phí duy trì hằng tháng
    total_hardware_price = Column(Integer, default=0)  # Tiền phần cứng
    upfront_payment = Column(Integer, default=0)  # Tiền trả trước (Trả thẳng / Trả góp)
    remaining_payment = Column(Integer, default=0)  # Tiền còn lại (Nếu trả góp)
    payment_method = Column(String(50), nullable=True)  # 'Trực tiếp' hoặc 'Trả góp'
    payment_status = Column(String(50), default="Chưa thanh toán")  # Trạng thái hóa đơn
...
class ProjectMember(Base):
    ...
    role = Column(String(50), default="Khách truy cập") # Super Admin, Unit Manager, Subject Teacher
    unit = Column(String(100), nullable=True) # Lớp chủ nhiệm (nếu có)
    teaching_subject = Column(String(100), nullable=True) # Môn giảng dạy (VD: Toán, Văn...)
    status = Column(String(50), default="Đang xét duyệt")
